...
| `SERVICE_MANAGEMENT_REFERENCE` | no | unset | Set to a service-tree GUID if your tenant requires `serviceManagementReference` on every new app registration (common on Microsoft corp tenants). The preprovision hook passes it to `az ad app create` for both the Easy Auth and the Bot app registrations. |
...
| `[preprovision] ERROR: Failed to create ... app registration` and `ServiceManagementReference field is required for Create` | Restricted tenant requires `serviceManagementReference` (service-tree GUID) on every new app registration. | `azd env set SERVICE_MANAGEMENT_REFERENCE <guid>` and re-run `devclaw up`. The hook forwards it to both `az ad app create` calls (Easy Auth + Bot). Get the GUID from your tenant admin or service tree. |
